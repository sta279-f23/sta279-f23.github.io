...
df2_new = df2.melt(id_vars = 'id', var_name = 'measurement', value_name = 'value')
df2_new
```
...
df2_new = df2.melt(id_vars = 'id', var_name = 'measurement', value_name = 'value')
df2_new[['measurement', 'stage']] = df2_new['measurement'].str.split('_', expand=True)
df2_new
```
...
air_quality = air_quality.query('parameter == "no2"').filter(['date.utc', 'location', 'value'])
```
...
```{python}
air_quality.pivot(index = 'date.utc', columns = 'location', values = 'value')
```
