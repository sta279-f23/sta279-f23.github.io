...
**Instructions:** Work with a neighbor to answer the following questions. To get started, download the [class activity template](https://sta279-f23.github.io/class_activities/ca_lecture_13_template.qmd) file.
...
1. Modify the code above to select the columns `x1`, `x2`, and `x3`, *without* listing them explicitly.
...
```
2. Modify the code above to select only the *numeric* columns (hint: use `is.numeric`).

3. Now select only the numeric columns which start with `x` (hint: use `&`)
...
4. Modify the code above to calculate the median and IQR for the numeric columns which start with `x`. Do not list the columns explicitly.
